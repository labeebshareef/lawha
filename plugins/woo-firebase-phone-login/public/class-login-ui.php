...
class Login_UI {
    ...
    public function render_phone_login() {
        if ( is_user_logged_in() || ! Helpers::is_firebase_configured() ) {
            return;
        }
    ...
    /**
     * Render a floating popup trigger button.
     *
     * Only rendered when popup mode is enabled, so users do not need
     * to add a [data-wfpl-popup] attribute to their theme manually.
     */
    public function render_popup_trigger() {
        if ( is_user_logged_in() || ! Helpers::is_firebase_configured() ) {
            return;
        }
        ?>
        <button type="button" id="wfpl-popup-fab" data-wfpl-popup
                aria-label="<?php esc_attr_e( 'Login with Phone', 'woo-firebase-phone-login' ); ?>"
                style="
                    position:fixed;bottom:24px;right:24px;z-index:9998;
                    width:56px;height:56px;border-radius:50%;border:none;cursor:pointer;
                    background:linear-gradient(135deg,#6c5ce7 0%,#a29bfe 100%);
                    color:#fff;box-shadow:0 4px 20px rgba(108,92,231,.4);
                    display:flex;align-items:center;justify-content:center;
                    transition:transform .25s ease,box-shadow .25s ease;
                "
                onmouseenter="this.style.transform='scale(1.1)';this.style.boxShadow='0 6px 28px rgba(108,92,231,.5)'"
                onmouseleave="this.style.transform='scale(1)';this.style.boxShadow='0 4px 20px rgba(108,92,231,.4)'"
        >
