/**
 * Phone Auth — Account Authentication UI
 *
 * Handles:
 ...
 *
 * Dependencies: jQuery, WFPL (phone-auth-sdk), intl-tel-input
 * Config:       phoneAuthConfig (localized by Asset_Loader)
 *
 * @package PhoneAuth
 */

/* global jQuery, WFPL, phoneAuthConfig, wfpl_config, intlTelInput */

(function ($) {
    'use strict';

    // Unified config source.
    var config = (typeof phoneAuthConfig !== 'undefined') ? phoneAuthConfig
    ...
    function createTimer($timerEl, $resendBtn) {
        ...
        function formatTime(seconds) {
            ...
            var tmpl = (config && config.i18n && config.i18n.resend_in) || 'Resend in %s';
        ...
        return {
            start: function () {
                this.clear();
                var seconds = (config && parseInt(config.otp_expiration, 10)) || 120;
                $resendBtn.hide();
    ...
    (function initRegPhone() {
        ...
        $sendBtn.on('click', async function () {
            ...
            try {
                ...
                console.error('[PhoneAuth Reg OTP]', err);
        ...
        $verifyBtn.on('click', async function () {
            ...
            try {
                ...
                console.error('[PhoneAuth Reg OTP Verify]', err);
                showMsg(friendlyError(err), 'error');
    ...
    (function initForgotPassword() {
        ...
        $sendBtn.on('click', async function () {
            ...
            try {
                var checkResult = await $.ajax({
                    ...
                    method: 'POST',
                    data: {
                        action: 'phone_auth_forgot_check',
                        nonce:  config.nonce,
                        phone:  phone
                    }
                });
                ...
                console.error('[PhoneAuth Forgot OTP]', err);
                showMsg(friendlyError(err), 'error');
            } finally {
                setLoading($sendBtn, false);
            }
        ...
        $verifyBtn.on('click', async function () {
            ...
            try {
                ...
                console.error('[PhoneAuth Forgot Verify]', err);
                showMsg(friendlyError(err), 'error');
                resetOtpInputs($digits);
                $digits.first().focus();
            } finally {
        ...
        $resetPwBtn.on('click', async function () {
            ...
            try {
                var result = await $.ajax({
                    ...
                    method: 'POST',
                    data: {
                        action:       'phone_auth_forgot_reset',
                        nonce:        config.nonce,
                        phone:        forgotPhone,
                        new_password: newPw,
                ...
                console.error('[PhoneAuth Forgot Reset]', err);
    ...
    (function initEmailVerify() {
        ...
        // Email verification uses the theme's lawha_wc_nonce (from lawhaWC).
        var wcConfig = (typeof lawhaWC !== 'undefined') ? lawhaWC : null;
        ...
        $resendBtn.on('click', function () {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    nonce:  wcConfig ? wcConfig.nonce : config.nonce
                },
